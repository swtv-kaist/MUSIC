...
void VLAR::setRange(std::set<std::string> &range)
{
...
void VLAR::Mutate(clang::Expr *e, MusicContext *context)
{
	...
	// get all variable declaration that VLAR can mutate this expr to.
  VarDeclList range(
...
bool VLAR::HandleRangePartition(string option) 
{
  vector<string> words;
...
void VLAR::ApplyRangePartition(vector<string> *range)
{
