...
bool RGAR::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
  // return domain.empty();
}

bool RGAR::ValidateRange(const std::set<std::string> &range)
{
  // for (auto e: range)
...
void RGAR::setRange(std::set<std::string> &range)
{
...
bool RGAR::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
...
bool RGAR::IsInitMutationTarget(clang::Expr *e, MusicContext *context)
{
...
void RGAR::Mutate(clang::Expr *e, MusicContext *context)
{
  BinaryOperator *bo;
  Expr *rhs;
  ...
  for (auto vardecl: *(context->getSymbolTable()->getGlobalArrayVarDeclList()))
  {
    ...
    if (token.compare(mutated_token) != 0 && 
        sameArrayElementType(rhs->getType(), vardecl->getType()))
    {
...
bool RGAR::HandleRangePartition(string option) 
{
  vector<string> words;
...
void RGAR::ApplyRangePartition(vector<string> *range)
{
