...
bool RLTR::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool RLTR::ValidateRange(const std::set<std::string> &range)
{
  // for (auto e: range)
...
void RLTR::setRange(std::set<std::string> &range)
{
...
bool RLTR::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
...
void RLTR::Mutate(clang::Expr *e, MusicContext *context)
{
  BinaryOperator *bo;
  Expr *rhs;
  ...
  // get all variable declaration that RLTR can mutate this expr to.
  VarDeclList range(
...
bool RLTR::HandleRangePartition(string option) 
{
  vector<string> words;
...
void RLTR::ApplyRangePartition(vector<string> *range)
{
