...
/* The domain for ArgStcDif must be names of functions whose
   function calls will be mutated. */
bool ArgStcDif::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool ArgStcDif::ValidateRange(const std::set<std::string> &range)
{
  // for (auto e: range)
...
void ArgStcDif::setRange(std::set<std::string> &range)
{
...
bool ArgStcDif::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
...
void ArgStcDif::Mutate(clang::Expr *e, MusicContext *context)
{
  CallExpr *ce;
