...
void RGCR::setRange(std::set<std::string> &range)
{
...
bool RGCR::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool RGCR::ValidateRange(const std::set<std::string> &range)
{
  return true;
}

// Return True if the mutant operator can mutate this expression
bool RGCR::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
...
bool RGCR::IsInitMutationTarget(clang::Expr *e, MusicContext *context)
{
...
void RGCR::Mutate(clang::Expr *e, MusicContext *context)
{
  // cout << "RGCR mutating" << endl;
  BinaryOperator *bo;
  Expr *rhs;
...
bool RGCRSortFunction (long long i,long long j) { return (i<j); }
bool RGCRSortFloatFunction (long double i,long double j) { return (i<j); }
    ...
    Expr *e, MusicContext *context, vector<string> *range)
{
  ...
  // cout << token << ": skip? " << skip_float_literal << endl;
  // PrintRange(context->comp_inst_->getSourceManager(), *stmt_context.non_floating_expr_range_);
  ...
  sort(range_values_int.begin(), range_values_int.end(), RGCRSortFunction);
...
void RGCR::MergeListsToStringList(vector<long long> &range_values_int,
                                  vector<long double> &range_values_float,
                                  vector<string> &range_values)
{
  int int_idx = 0;
...
bool RGCR::HandleRangePartition(string option) 
{
  vector<string> words;
