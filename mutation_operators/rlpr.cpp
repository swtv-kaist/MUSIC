...
bool RLPR::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool RLPR::ValidateRange(const std::set<std::string> &range)
{
  // for (auto e: range)
...
void RLPR::setRange(std::set<std::string> &range)
{
...
bool RLPR::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
...
void RLPR::Mutate(clang::Expr *e, MusicContext *context)
{
  cout << "RLPR mutates" << endl;
  ...
  cout << "mutating " << token << endl;

  // get all variable declaration that RLPR can mutate this expr to.
  VarDeclList range(
  ...
  for (auto vardecl: range)
  {
    ...
    cout << pointee_type << ":" << getPointerType(vardecl->getType()) << endl;
...
bool RLPR::HandleRangePartition(string option) 
{
  vector<string> words;
...
void RLPR::ApplyRangePartition(vector<string> *range)
{
