...
#include "dirvararineg.h"

bool DirVarAriNeg::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool DirVarAriNeg::ValidateRange(const std::set<std::string> &range)
{
  return true;
}

// Return True if the mutant operator can mutate this expression
bool DirVarAriNeg::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
  ...
  // DirVarAriNeg can mutate expr that are
...
bool DirVarAriNeg::IsDirVar(clang::Expr *e, MusicContext *context)
{
  ...
  // cout << "function id " << context->getFunctionId() << endl;
  // cout << (*(context->getSymbolTable()->getFuncUsedGlobalList())).size() << endl;
