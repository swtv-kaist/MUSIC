...
bool ORSN::ValidateDomain(const std::set<std::string> &domain)
{
...
bool ORSN::ValidateRange(const std::set<std::string> &range)
{
...
void ORSN::setDomain(std::set<std::string> &domain)
{
	if (domain.empty())
		domain_ = {">", "<", "<=", ">=", "==", "!="};
	else
...
void ORSN::setRange(std::set<std::string> &range)
{
	if (range.empty())
		range_ = {"<<", ">>"};
	else
...
bool ORSN::CanMutate(clang::Expr *e, ComutContext *context)
{
...
bool ORSN::CanMutate(clang::Stmt *s, ComutContext *context)
{
	return false;
}

void ORSN::Mutate(clang::Expr *e, ComutContext *context)
{
	BinaryOperator *bo;
...
void ORSN::Mutate(clang::Stmt *s, ComutContext *context)
{}

bool ORSN::CanMutate(BinaryOperator *bo, string mutated_token,
										 ComutContext *context)
{
	...
	// shift operator only takes integral operands
