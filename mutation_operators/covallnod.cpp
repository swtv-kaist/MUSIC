...
/* The domain for CovAllNod must be names of functions whose
   function calls will be mutated. */
bool CovAllNod::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool CovAllNod::ValidateRange(const std::set<std::string> &range)
{
  // for (auto e: range)
...
void CovAllNod::setRange(std::set<std::string> &range) {}

// Return True if the mutant operator can mutate this expression
bool CovAllNod::IsMutationTarget(clang::Stmt *s, MusicContext *context)
{
...
void CovAllNod::Mutate(clang::Stmt *s, MusicContext *context)
{ 
...
bool CovAllNod::NoUnremovableLabelInsideRange(
