...
#include "indvararineg.h"

bool IndVarAriNeg::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool IndVarAriNeg::ValidateRange(const std::set<std::string> &range)
{
  return true;
}

// Return True if the mutant operator can mutate this expression
bool IndVarAriNeg::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
  ...
  // IndVarAriNeg can mutate expr that are
...
bool IndVarAriNeg::IsIndVar(clang::Expr *e, MusicContext *context)
{
  ...
  {
    // cout << "candidate: " << vardecl->getNameAsString() << endl;
