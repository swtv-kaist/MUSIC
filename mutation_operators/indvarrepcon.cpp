...
#include "indvarrepcon.h"

bool IndVarRepCon::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool IndVarRepCon::ValidateRange(const std::set<std::string> &range)
{
  return true;
}

// Return True if the mutant operator can mutate this expression
bool IndVarRepCon::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
  // cout << "checking " << ConvertToString(e, context->comp_inst_->getLangOpts()) << endl;
  if (!ExprIsScalar(e) || !IsIndVar(e, context))
    return false;
  // cout << "cp1" << endl;
  ...
  // IndVarRepCon can mutate expr that are
...
void IndVarRepCon::Mutate(clang::Expr *e, MusicContext *context)
{
...
bool IndVarRepCon::IsIndVar(clang::Expr *e, MusicContext *context)
{
