...
#include "indvarreppar.h"

bool IndVarRepPar::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
}

bool IndVarRepPar::ValidateRange(const std::set<std::string> &range)
{
  return true;
}

// Return True if the mutant operator can mutate this expression
bool IndVarRepPar::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
  ...
  // IndVarRepPar can mutate expr that are
...
void IndVarRepPar::Mutate(clang::Expr *e, MusicContext *context)
{
  ...
  for (auto it: (*(context->getSymbolTable()->getFuncParamList()))[context->getFunctionId()])
  {
...
bool IndVarRepPar::IsIndVar(clang::Expr *e, MusicContext *context)
{
