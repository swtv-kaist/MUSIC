...
bool RGSR::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
...
bool RGSR::ValidateRange(const std::set<std::string> &range)
{
  // for (auto e: range)
...
void RGSR::setRange(std::set<std::string> &range)
{
...
bool RGSR::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
...
void RGSR::Mutate(clang::Expr *e, MusicContext *context)
{
  BinaryOperator *bo;
  Expr *rhs;
...
bool RGSR::HandleRangePartition(string option) 
{
  vector<string> words;
...
void RGSR::ApplyRangePartition(vector<string> *range)
{
