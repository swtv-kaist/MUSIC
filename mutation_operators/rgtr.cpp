...
bool RGTR::ValidateDomain(const std::set<std::string> &domain)
{
  return true;
...
bool RGTR::ValidateRange(const std::set<std::string> &range)
{
  // for (auto e: range)
...
void RGTR::setRange(std::set<std::string> &range)
{
...
bool RGTR::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
  if (BinaryOperator *bo = dyn_cast<BinaryOperator>(e))
  {
    ...
    return context->IsRangeInMutationRange(SourceRange(start_loc, end_loc)) &&
           !stmt_context.IsInEnumDecl() && is_in_domain && ExprIsStruct(rhs);
  }

  return false;
}

bool RGTR::IsInitMutationTarget(clang::Expr *e, MusicContext *context)
{
  ...
  return context->IsRangeInMutationRange(SourceRange(start_loc, end_loc)) &&
         !stmt_context.IsInEnumDecl() && is_in_domain && ExprIsStruct(e);
}

void RGTR::Mutate(clang::Expr *e, MusicContext *context)
{
  BinaryOperator *bo;
  Expr *rhs;
  ...
  string struct_type = getStructureType(rhs->getType());
...
bool RGTR::HandleRangePartition(string option) 
{
  vector<string> words;
...
void RGTR::ApplyRangePartition(vector<string> *range)
{
