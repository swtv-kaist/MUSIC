...
bool NEW_SSDL::CanMutate(Expr *e, ComutContext *context)
{
  return false;
}

void NEW_SSDL::Mutate(clang::Expr *e, ComutContext *context) 
{}

bool NEW_SSDL::ValidateDomain(const set<string> &domain)
{
  if (domain.empty())
    return true;
...
bool NEW_SSDL::ValidateRange(const set<string> &range)
{
  if (range.empty())
    return true;
...
bool NEW_SSDL::CanMutate(Stmt *s, ComutContext *context)
{
	...
	// Hence it is better to check in NEW_SSDL::Mutate function.
...
void NEW_SSDL::Mutate(Stmt *s, ComutContext *context)
{
	CompoundStmt *c;
...
void NEW_SSDL::DeleteStatement(Stmt *s, ComutContext *context)
{
...
bool NEW_SSDL::NoUnremovableLabelInsideRange(
...
bool NEW_SSDL::HandleStmtWithSubStmt(Stmt *s, ComutContext *context)
{
...
void NEW_SSDL::HandleStmtWithBody(Stmt *s, ComutContext *context)
{
	Stmt *body = 0;
...
void NEW_SSDL::DeleteCompoundStmtContent(CompoundStmt *c, ComutContext *context)
{
