...
bool OSBA::ValidateDomain(const std::set<std::string> &domain)
{
...
bool OSBA::ValidateRange(const std::set<std::string> &range)
{
...
void OSBA::setDomain(std::set<std::string> &domain)
{
	if (domain.empty())
		domain_ = {"<<=", ">>="};
	else
...
void OSBA::setRange(std::set<std::string> &range)
{
	if (range.empty())
		range_ = {"&=", "|=", "^="};
	else
...
bool OSBA::CanMutate(clang::Expr *e, ComutContext *context)
{
...
bool OSBA::CanMutate(clang::Stmt *s, ComutContext *context)
{
	return false;
}

void OSBA::Mutate(clang::Expr *e, ComutContext *context)
{
	BinaryOperator *bo;
...
void OSBA::Mutate(clang::Stmt *s, ComutContext *context)
{}