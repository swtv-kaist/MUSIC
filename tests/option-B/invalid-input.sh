#!/bin/sh
# When user provides something that an operator cannot mutate for option -B
...
do
    ...
    do
    	...
    	echo "../../tool input-src/${TEST_INPUT} -o $OUTPUT_FOLDER_NAME $LINE"
    	../../tool input-src/${TEST_INPUT} -o $OUTPUT_FOLDER_NAME $LINE > /dev/null 2>&1
		...
		then
		    echo "[SUCCESS ${COUNT}/41] $TEST_INPUT invalid range option"
		else
		    echo "[FAIL ${COUNT}/41] $TEST_INPUT invalid range option"
		fi
