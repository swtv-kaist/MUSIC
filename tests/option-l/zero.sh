#!/bin/sh
# COMUT exits on error in whic users provide 0 for option -l
...
do
    ...
    ../../tool input-src/${TEST_INPUT} -o $OUTPUT_FOLDER_NAME -l 0 > /dev/null 2>&1
    ...
    then
        echo "[SUCCESS] $TEST_INPUT input is 0"
    else
        echo "[FAIL] $TEST_INPUT input is 0"
    fi
