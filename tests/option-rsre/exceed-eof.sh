...
test_exceed_eof()
{
    ...
    echo "../../tool input-src/${TEST_INPUT} -o $OUTPUT_FOLDER_NAME $1 $2"

    # Run the tool with the input source and start of mutation range exceeding EOF
    ../../tool input-src/${TEST_INPUT} -o $OUTPUT_FOLDER_NAME $1 $2 > /dev/null 2>&1
...
do
    ...
    test_exceed_eof "-rs" "$LAST_LINE_NUMBER $((3+LAST_LINE_COL_NUMBER))"
    test_exceed_eof "-rs" "$((1+$LAST_LINE_NUMBER)) $((3+LAST_LINE_COL_NUMBER))"
    test_exceed_eof "-re" "$LAST_LINE_NUMBER $((3+LAST_LINE_COL_NUMBER))"
    test_exceed_eof "-re" "$((1+$LAST_LINE_NUMBER)) $((3+LAST_LINE_COL_NUMBER))" 
